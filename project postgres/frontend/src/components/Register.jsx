...
const Register = () => {
    ...
    const  {infos} =useSelector(state => state.employees);//전체 employee 가져오기

    const isDuplicate = infos.some(emp => emp.name === newEmployee.name);//중복확인
    // useEffect(() => {
    ...
    // const handleSubmit = e => {
    //     e.preventDefault();
    //     if(infos.find(info => info.name === newEmployee.name)){
    //         alert("존재하는 이름. 다시 작성할 것을 권장.")
    //         return;
    //     }
    //     dispatch(fetchPostEmployee(newEmployee));
